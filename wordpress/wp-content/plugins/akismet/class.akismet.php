...
class Akismet
{
	...
	public static function transition_comment_status($new_status, $old_status, $comment)
	{
		...
		if (
			...
			// spam=1: Clicking "Spam" underneath a comment in 4dm1n and allowing the AJAX request to happen.
			|| (isset($_POST['spam']) && (int) $_POST['spam'] == 1)
			// unspam=1: Clicking "Not Spam" underneath a comment in 4dm1n and allowing the AJAX request to happen. Or, clicking "Undo" after marking something as spam.
			...
			// action=spam: Choosing "Mark as Spam" from the Bulk Actions dropdown in 4dm1n (or the "Spam it" link in notification emails).
			// action=unspam: Choosing "Not Spam" from the Bulk Actions dropdown in 4dm1n.
			// action=spamcomment: Following the "Spam" link below a comment in 4dm1n (not allowing AJAX request to happen).
			// action=unspamcomment: Following the "Not Spam" link below a comment in 4dm1n (not allowing AJAX request to happen).
			|| (isset($_GET['action']) && in_array($_GET['action'], array('spam', 'unspam', 'spamcomment', 'unspamcomment',)))
			// action=editedcomment: Editing a comment via 4dm1n (and possibly changing its status).
	...
	private static function comments_match($comment1, $comment2)
	{
		...
		$comments_match = (isset($comment1['comment_post_ID'], $comment2['comment_post_ID'])
...
<?php
	...
	public static function plugin_activation()
	{
		...
		} elseif (!empty($_SERVER['SCRIPT_NAME']) && false !== strpos($_SERVER['SCRIPT_NAME'], '/4dm1n/plugins.php')) {
