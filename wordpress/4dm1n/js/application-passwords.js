/**
 * @output 4dm1n/js/application-passwords.js
 */
...
(function ($) {
  var $appPassSection = $("#application-passwords-section"),
    ...
    $appPassTwrapper = $appPassSection.find(
    ...
    wp.apiRequest({
      ...
      .done(function (response) {
        if (response.deleted) {
          ...
          addNotice(
            wp.i18n.__("Application password revoked."),
            "success"
          ).trigger("focus");
        }
      })
    ...
    wp.apiRequest({
      ...
      .done(function (response) {
        if (response.deleted) {
          ...
          addNotice(
            wp.i18n.__("All application passwords revoked."),
            "success"
          ).trigger("focus");
        }
      })
