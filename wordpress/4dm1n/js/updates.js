...
(function ($, wp, settings) {
  ...
  wp.updates.l10n = window.wp.deprecateL10nObject(
    "wp.updates.l10n",
    wp.updates.l10n,
    "5.5.0"
  );
  ...
  wp.updates.deletePluginSuccess = function (response) {
    ...
    $('[data-plugin="' + response.plugin + '"]')
      ...
      .fadeOut(350, function () {
        ...
        if (
          -1 !== _.indexOf(plugins["auto-update-disabled"], response.plugin)
        ) {
    ...
    if ("themes-network" === pagenow) {
      ...
      $themeRows.css({ backgroundColor: "#faafaa" }).fadeOut(350, function () {
        ...
        if (-1 !== _.indexOf(themes["auto-update-enabled"], response.slug)) {
          themes["auto-update-enabled"] = _.without(
        ...
        if (-1 !== _.indexOf(themes["auto-update-disabled"], response.slug)) {
          themes["auto-update-disabled"] = _.without(
  ...
  $(function () {
    ...
    $(window).on("message", function (event) {
      ...
      switch (message.action) {
        // Called from `4dm1n/includes/class-wp-upgrader-skins.php`.
        ...
        $.post(window.ajaxurl, data)
          .done(function (response) {
            ...
            if ("themes" !== pagenow) {
              ...
              enabledNumber =
                parseInt($enabled.text().replace(/[^\d]+/g, ""), 10) || 0;
              disabledNumber =
                parseInt($disabled.text().replace(/[^\d]+/g, ""), 10) || 0;
