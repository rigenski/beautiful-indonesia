...
function wp_get_revision_ui_diff($post, $compare_from, $compare_to)
{
	...
	foreach (_wp_post_revision_fields($post) as $field => $name) {
		...
		/** This filter is documented in 4dm1n/includes/revision.php */
...
?><script id="tmpl-revisions-frame" type="text/html">
	...
	<script id="tmpl-revisions-checkbox" type="text/html">
		<div class="revision-toggle-compare-mode">
			<label>
				<input type="checkbox" class="compare-two-revisions" <# if ( 'undefined' !==typeof data && data.model.attributes.compareTwoMode ) { #> checked="checked"<# } #>
					/>
						...
						<# } #>
							<div class="author-card<# if ( data.attributes.autosave ) { #> autosave<# } #>">
								...
								<# if ( 'to'===data.type && data.attributes.restoreUrl ) { #>
									<input <?php if (wp_check_post_lock($post->ID)) { ?> disabled="disabled" <?php } else { ?> <# if ( data.attributes.current ) { #>
									disabled="disabled"
									<# } #>
									<?php } ?>
									<# if ( data.attributes.autosave ) { #>
