...
if (is_admin()) {
	// 4dm1n pages are checked more carefully.
	if (is_network_admin()) {
		preg_match('#/4dm1n/network/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches);
	} elseif (is_user_admin()) {
		preg_match('#/4dm1n/user/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches);
	} else {
		preg_match('#/4dm1n/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches);
	}
	...
	if ('' === $pagenow || 'index' === $pagenow || 'index.php' === $pagenow) {
		...
		if ('.php' !== substr($pagenow, -4, 4)) {
			$pagenow .= '.php'; // For `Options +Multiviews`: /4dm1n/themes/index.php (themes.php is queried).
		}
	}
} else {
